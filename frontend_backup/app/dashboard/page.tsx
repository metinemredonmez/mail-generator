...
function StatCard({ title, value, icon, color, bgColor, subtitle, loading }: StatCardProps) {
  return (
    <Card>
      <CardContent>
  ...
  return (
    <Paper
      ...
      sx={(theme) => ({
        ...
        gap: 2,
        textDecoration: 'none',
        bgcolor: theme.palette.mode === 'dark' ? 'background.paper' : bgColor,
        borderRadius: 2,
        transition: 'all 0.2s',
...
export default function DashboardPage() {
  const { data: passengerStats, isLoading: passengerLoading } = useQuery({
  ...
  const { data: recentCodes, isLoading: codesLoading } = useQuery({
    queryKey: ['recent-codes'],
    queryFn: () => inboxApi.getAllCodes({ limit: 5 }).then((res) => res.data),
  });

  const statsLoading = passengerLoading || emailLoading || groupLoading;
  ...
  const handleRefresh = async () => {
    ...
    try {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['recent-codes'] }),
      ]);
    } finally {
    ...
    <FullLayout>
      <Box>
        ...
        >
          ...
          >
            {isRefreshing ? <CircularProgress size={20} /> : <IconRefresh size={20} />}
          </IconButton>
        </Stack>
